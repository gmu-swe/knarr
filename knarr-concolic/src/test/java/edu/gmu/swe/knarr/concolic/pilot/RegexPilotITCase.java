...
/**
 * Pilot 7 ITCase — drives {@link RegexPilotTarget} under the dual JDK
 * through the same five (six w/ {@code KNARR_LLM_RANKER}) mutators as
 * {@link AntPilotITCase}. Anchors the ReDoS / catastrophic-backtracking
 * CVE tail (CVE-2017-16223 family); the seed {@code (a+)+$} offers an
 * ideal concolic flip between "simple regex" and
 * "backtracking-pathological" branches.
 */
public class RegexPilotITCase {

    @Test
    void regexCompileMatchPilot() throws Exception {
        ...
        try (PilotHarness.Server ignored =
            ...
            for (String mutator : mutators) {
                ...
                PilotHarness.snapshot("regex-pilot-" + mutator + "-output.txt", out);
                PilotHarness.RunResult r = PilotHarness.parseRun("REGEX", out);
                results.put(mutator, r);
                System.out.println("[RegexPilotITCase] " + mutator
                        + " iters=" + r.iters
        ...
        PilotHarness.writeTable("regex-pilot-table.txt", results);
