...
if [ -z "$JAVA_HOME" ]; then
	...
	if [ ! -d "target/jre-inst" ]; then
		...
		java -DspecialStrings=equals,startsWith,indexOf,contains  -DaddCov="javax/imageio" $DEBUG -Xmx6g -jar target/Knarr-0.0.2-SNAPSHOT.jar $INST_HOME target/jre-inst;
